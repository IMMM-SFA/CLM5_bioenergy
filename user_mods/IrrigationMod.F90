...
contains
  ...
  subroutine IrrigationInitHistory(this, bounds)
	...
	! Y. Cheng, March 11th, 2019. change to active
  ...
  subroutine ApplyIrrigation(this, bounds)
    ...
    do p = bounds%begp, bounds%endp
       g = patch%gridcell(p)
       !print *, "apply irrigation checking",p,patch%gridcell(p),bounds%begp,bounds%endp
  ...
  subroutine CalcIrrigationNeeded(this, bounds, num_exposedvegp, filter_exposedvegp, &
    ...
    do fp = 1, num_exposedvegp
       p = filter_exposedvegp(fp)
       c = patch%column(p)
       !print *, "1CalcIrrigationNeeded checking:",fp,num_exposedvegp,p,c,this%irrig_rate_patch(p)
       ...
       if (patch%itype(p) == 71 .or. patch%itype(p)==73 .or. patch%itype(p) == 72 .or. patch%itype(p) == 74) then
          ...
          !print *,"check irrigation bioenergy yayaya:",patch%itype(p),p,this%irrig_rate_patch(p),this%irrig_rate_demand_patch(p)
       end if
       
       !print *, "2CalcIrrigationNeeded checking:",fp,num_exposedvegp,p,c,this%irrig_rate_patch(p)
