...
!                if (tlai(p)>0) then
!                    print *, "tsai in CNVegStrucUpdate check:",p,ivt(p),tlai(p),tsai(p),laimx(ivt(p)),jday,peaklai(p)
!                end if
...
!                if (tlai(p)>0) then
!                    print *, "tsai in CNVegStrucUpdate check1:",p,ivt(p),tlai(p),tsai(p),laimx(ivt(p)),jday,harvdate(p),farea_burned(c),htop(p),peaklai(p)
!                end if
