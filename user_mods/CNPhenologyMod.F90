...
!        print *, "before calling CNOffsetLitterfall leafc_patch", cnveg_carbonstate_inst%leafc_patch
!        print *, "before calling CNOffsetLitterfall grainc_patch", cnveg_carbonstate_inst%grainc_patch
!        print *, "before calling CNOffsetLitterfall grainc_to_food_patch", cnveg_carbonflux_inst%grainc_to_food_patch
!        print *, "before calling CNOffsetLitterfall grainn_patch", cnveg_nitrogenstate_inst%grainn_patch
!        print *, "before calling CNOffsetLitterfall leafn_patch", cnveg_nitrogenstate_inst%leafn_patch
!        print *, "before calling CNOffsetLitterfall stemc_patch", cnveg_carbonstate_inst%livestemc_patch
!        print *, "before calling CNOffsetLitterfall stemn_patch", cnveg_nitrogenstate_inst%livestemn_patch

       call CNOffsetLitterfall(num_soilp, filter_soilp, &
            cnveg_state_inst, cnveg_carbonstate_inst, cnveg_nitrogenstate_inst, cnveg_carbonflux_inst, cnveg_nitrogenflux_inst)
             
!        print *, "after calling CNOffsetLitterfall leafc_patch", cnveg_carbonstate_inst%leafc_patch
!        print *, "after calling CNOffsetLitterfall grainc_patch", cnveg_carbonstate_inst%grainc_patch
!        print *, "after calling CNOffsetLitterfall grainc_to_food_patch", cnveg_carbonflux_inst%grainc_to_food_patch
!        print *, "after calling CNOffsetLitterfall grainn_patch", cnveg_nitrogenstate_inst%grainn_patch
!        print *, "after calling CNOffsetLitterfall leafn_patch", cnveg_nitrogenstate_inst%leafn_patch
!        print *, "after calling CNOffsetLitterfall stemc_patch", cnveg_carbonstate_inst%livestemc_patch
!        print *, "after calling CNOffsetLitterfall stemn_patch", cnveg_nitrogenstate_inst%livestemn_patch
  ...
  subroutine CNPhenologyInit(bounds)
    ...
    !print *, 'fstor2tran is equal to:', fstor2tran
...
!                   print *, "gddmaturity check1:",gddmaturity(p),p,jday,maxplantjday(ivt(p),h),ivt(p)
!                   print *, "gddmaturity check11:",gdd020(p),hybgdd(ivt(p)),gdd820(p),p,num_pcropp
! 				  print *, "gddmaturity check111:",ntmp_corn,ntmp_soybean,nirrig_tmp_corn,nirrig_trp_corn,ntrp_corn
...
! 				  print *, "gddmaturity check2:",gddmaturity(p),p,jday,maxplantjday(ivt(p),h),ivt(p)
!                   print *, "gddmaturity check22:",gdd020(p),hybgdd(ivt(p)),gdd820(p),p,num_pcropp
! 				  print *, "gddmaturity check222:",ntmp_corn,ntmp_soybean,nirrig_tmp_corn,nirrig_trp_corn,ntrp_corn
...
!             print *, "final gddmatrurity check:",gddmaturity(p),p,ivt(p),num_pcropp
         ...
         if (croplive(p)) then
            ...
            if (leafout(p) >= huileaf(p) .and. hui(p) < huigrain(p) .and. idpp < mxmat(ivt(p))) then
               ...
               if (abs(onset_counter(p)) > 1.e-6_r8) then
                  ...
                  onset_counter(p) = dt
                    ...
                    if (ivt(p)==71 .or. ivt(p)==72) then
                       ...
                       !print *,"no fertilizer yayaya:",ivt(p),p
                    end if
              ...
              !print *, "harvest related variables before", hui(p), gddmaturity(p),idpp, mxmat(ivt(p)), harvdate(p), NOT_Harvested, jday
            !else if (hui(p) >= gddmaturity(p) .or. idpp >= mxmat(ivt(p))) then
            !else if (hui(p) >= 2400) then
            else if (idpp >= mxmat(ivt(p))) then
               !print *, "check mxmat values",mxmat(ivt(p)),p,idpp,jday,num_pcropp
               if (harvdate(p) >= NOT_Harvested) harvdate(p) = jday
               croplive(p) = .false.     ! no re-entry in greater if-block
               cphase(p) = 4._r8
               !print *, "harvest related variables after", hui(p), gddmaturity(p),idpp, mxmat(ivt(p)), harvdate(p), NOT_Harvested, jday
...
!        print *,"is pft known to model1:",n,npcropmin, npcropmax,pftcon%is_pft_known_to_model(n),minplantjday(n, inNH),maxplantjday(n, inNH)
...
!        print *,"is pft known to model2:",n,npcropmin, npcropmax,pftcon%is_pft_known_to_model(n),minplantjday(n, inNH),maxplantjday(n, inNH)
    end do
  ...
  subroutine CNOffsetLitterfall (num_soilp, filter_soilp, &
    ...
    associate(                                                                           & 
      ...
      do fp = 1,num_soilp
         p = filter_soilp(fp)
         !print *, "beginning of CNOffsetLitterfall", p, fp, num_soilp, filter_soilp(fp), offset_flag(p),offset_counter(p),jday,npcropmin, use_fun
         !print *, "beginning of CNOffsetLitterfall values", leafc(p), leafn(p),  grainc_to_food(p), grainc(p),  grainn_to_food(p), grainn(p), leafc_to_litter(p),livestemn(p)
...
!                print *,"Offset cut check:",p,fp,num_soilp,ivt(p)
               ...
               if (ivt(p) >= npcropmin) then
                  ...
                  !print *, 'grainc_to_food before:', grainc_to_food(p),t1, grainc(p), cpool_to_grainc(p) ,grainc_to_seed(p)
                  ...
                  !print *, 'grainc_to_food after :', grainc_to_food(p),t1, grainc(p), cpool_to_grainc(p) ,grainc_to_seed(p)


                  ! Original code
                  livestemc_to_litter(p) = t1 * livestemc(p) + cpool_to_livestemc(p)
 
                ! Y. Cheng, cut 70% stem to grain_to_food
!                   livestemc_to_litter(p) = t1 * livestemc(p)*0.3 + cpool_to_livestemc(p)
!                   grainc_to_food(p) = grainc_to_food(p) +t1 * livestemc(p) * 0.7
!                   grainn_to_food(p) = grainn_to_food(p) +t1 * livestemn(p) * 0.7   
...
!                grainc_to_food(p) = grainc_to_food(p) + t1 * livestemc(p) * 0.7
!                grainn_to_food(p) = grainn_to_food(p) + t1 * livestemn(p) * 0.7
!                grainc(p) = grainc(p) + livestemc(p)*0.7
!                livestemc(p)  = livestemc(p) * 0.1         
!                grainn(p) = grainn(p) + livestemn(p)*0.7
!                livestemn(p)  = livestemn(p) * 0.1               
	...
	!                if (ivt(p) >= npcropmin) then
	!                	grainc(p) = grainc(p) + livestemc(p)*0.7
	!               	livestemc(p)  = livestemc(p) * 0.3
	!                	grainn(p) = grainn(p) + livestemn(p)*0.7
	!                	livestemn(p)  = livestemn(p) * 0.3
	!                end if
			 end if
         end if 
...
!          else  ! me add this, Oct 16th, 2018
!            if (ivt(p) >= npcropmin) then
!                if (jday > 165 .and. jday < 175) then
! 					   print *, "beginning of CNOffsetLitterfall values not offset use jday", p, fp, num_soilp, filter_soilp(fp), offset_flag(p),offset_counter(p),jday,npcropmin, use_fun
! 					   print *, "begin of CNOffsetLitterfall values not offset use jday", p, leafc(p), leafn(p),  grainc_to_food(p), grainc(p),  grainn_to_food(p), grainn(p)
! 		   
! 					   t1 = 1.0_r8 / dt
! 					   grainc_to_food(p) = grainc_to_food(p) +t1 * leafc(p) * (1-0.995)
! 					   grainn_to_food(p) = grainn_to_food(p) +t1 * leafn(p) * (1-0.995)
! 		   
! 					   grainc(p) = grainc(p) + leafc(p)*(1-0.995)
! 					   leafc(p)  = leafc(p) * 0.995
! 					   grainn(p) = grainn(p) + leafn(p)*(1-0.995)
! 					   leafn(p)  = leafn(p) * 0.995       
! 					   print *, "end of CNOffsetLitterfall values not offset", p ,leafc(p), leafn(p),  grainc_to_food(p), grainc(p),  grainn_to_food(p), grainn(p)
! 				
! 				else
! 				  if (leafc(p) > 60) then
! 					   print *, "beginning of CNOffsetLitterfall values not offset", p, fp, num_soilp, filter_soilp(fp), offset_flag(p),offset_counter(p),jday,npcropmin, use_fun
! 					   print *, "begin of CNOffsetLitterfall values not offset", p, leafc(p), leafn(p),  grainc_to_food(p), grainc(p),  grainn_to_food(p), grainn(p)
! 		   
! 					   t1 = 1.0_r8 / dt
! 					   grainc_to_food(p) = grainc_to_food(p) +t1 * leafc(p) * (1-0.995)
! 					   grainn_to_food(p) = grainn_to_food(p) +t1 * leafn(p) * (1-0.995)
...
!                 if (jday > 165 .and. jday < 175) then
! 					   print *, "beginning of CNOffsetLitterfall values not offset", p, fp, num_soilp, filter_soilp(fp), offset_flag(p),offset_counter(p),jday,npcropmin, use_fun
! 					   print *, "begin of CNOffsetLitterfall values not offset", p, livestemc(p), livestemn(p),  grainc_to_food(p), grainc(p),  grainn_to_food(p), grainn(p)
! 		   
! 					   t1 = 1.0_r8 / dt
! 					   grainc_to_food(p) = grainc_to_food(p) +t1 * livestemc(p) * (1-0.995)
! 					   grainn_to_food(p) = grainn_to_food(p) +t1 * livestemn(p) * (1-0.995)
! 		   
! 					   grainc(p) = grainc(p) + livestemc(p)*(1-0.995)
! 					   livestemc(p)  = livestemc(p) * 0.995
! 					   grainn(p) = grainn(p) + livestemn(p)*(1-0.995)
! 					   livestemn(p)  = livestemn(p) * 0.995       
! 					   print *, "end of CNOffsetLitterfall values not offset", p ,livestemc(p), livestemn(p),  grainc_to_food(p), grainc(p),  grainn_to_food(p), grainn(p)
! 				
! 				else
! 				  if (livestemc(p) > 100) then
! 					   print *, "beginning of CNOffsetLitterfall values not offset", p, fp, num_soilp, filter_soilp(fp), offset_flag(p),offset_counter(p),jday,npcropmin, use_fun
! 					   print *, "begin of CNOffsetLitterfall values not offset", p, livestemc(p), leafn(p),  grainc_to_food(p), grainc(p),  grainn_to_food(p), grainn(p)
! 		   
! 					   t1 = 1.0_r8 / dt
! 					   grainc_to_food(p) = grainc_to_food(p) +t1 * livestemc(p) * (1-0.995)
! 					   grainn_to_food(p) = grainn_to_food(p) +t1 * livestemn(p) * (1-0.995)
...
!            end if !(ivt(p) >= npcropmin)
         ...
         !print *, "end of CNOffsetLitterfall values", p, leafc(p), leafn(p),  grainc_to_food(p), grainc(p),  grainn_to_food(p), grainn(p), leafc_to_litter(p), livestemn(p)
